...
To install the Abjad IDE:
    ...
    2. add abjadide/scr/ to your PATH.
...
2. Add the abjadide/scr/ directory to your PATH. This tells your shell
where the start-abjad-idetools script is housed:

    export PATH=$ABJAD/abjadide/scr:$PATH
...
5. Run pytest against the abjadide directory. Fix or report tests that
break.

6. Build the Abjad IDE API. Run 'avj api -S' to do this.

7. Run doctest on the abjadide/ directory.  Run 'ajv doctest' in
abjadide/ directory to do this. You're ready to use the Abjad IDE when
all tests pass.
'''
import sys
