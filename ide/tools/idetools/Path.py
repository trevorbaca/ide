...
class Path(abjad.Path):
    ...
    def smart_activate(self, tag, deactivate=False):
        ...
        Returns nonnegative integer count of activated tags.
        '''
        if self.is_file():
            count = self._activate_tag_in_file(tag, deactivate=deactivate)
        elif self.is_segment():
            illustration_ly = self('illustration.ly')
            assert illustration_ly.is_file()
            count = illustration_ly.smart_activate(tag, deactivate=deactivate)
        elif self.is_segments():
            count = 0
            for segment in self.list_paths():
                count += segment.smart_activate(tag, deactivate=deactivate)
        elif self.is_build() or self.is__segments():
            count = 0
            for segment_ly in self.build._segments.list_paths():
                count += segment_ly.smart_activate(tag, deactivate=deactivate)
        else:
            raise ValueError(self)
        return count

    def smart_deactivate(self, tag):
        r'''Activates `tag` in path.

        Dual of smart_activate().

        Returns nonnegative integer count of deactivated tags.
        '''
        return self.smart_activate(tag, deactivate=True)

    def _activate_tag_in_file(self, tag, deactivate=False):
        import abjad
        text = self.read_text()
        if deactivate:
