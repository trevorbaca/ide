...
def test_AbjadIDE_illustrate_definition_01():
    ...
    with systemtools.FilesystemState(keep=[ly_path, pdf_path]):
        os.remove(ly_path)
        os.remove(pdf_path)
        input_ = 'red~example~score gg A i q'
...
def test_AbjadIDE_illustrate_definition_02():
...
def test_AbjadIDE_illustrate_definition_03():
    ...
    with systemtools.FilesystemState(keep=[ly_path, pdf_path]):
        ...
        input_ = 'red~example~score gg A i y q'
        abjad_ide._start_abjad_ide(input_=input_)
        assert os.path.isfile(ly_path)
        assert os.path.isfile(pdf_path)
        assert systemtools.TestManager._compare_backup(ly_path)
